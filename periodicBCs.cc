...
//vertices_DOFs: This index set has the global dofs for vertices.
  ...
  // Taking different values of subdivisions in each directions
  totalNumEachEdgesNodes=userInputs.subdivisions;
  totalNumEachEdgesNodes[0] = totalNumEachEdgesNodes[0] - 1;
  totalNumEachEdgesNodes[1] = totalNumEachEdgesNodes[1] - 1;
  totalNumEachEdgesNodes[2] = totalNumEachEdgesNodes[2] - 1;
...
//of an edge (for example the first row is the all dofs for EX000_1);
  edges_DOFs.resize(totalNumEdgesDOFs);
  for(unsigned int i=0;i<(totalNumEdgesDOFs/3);i++){
  edges_DOFs[i] = std::vector<unsigned int>(totalNumEachEdgesNodes[0],0); //rows 0-11 corresponde to edges in x-direction
  edges_DOFs[i+12] = std::vector<unsigned int>(totalNumEachEdgesNodes[1],0); //rows 12-23 corresponde to edges in y-direction
  edges_DOFs[i+24] = std::vector<unsigned int>(totalNumEachEdgesNodes[2],0);} //rows 24-35 corresponde to edges in z-direction
...
//Edges_DOFs_Coord_Vector: is a vector that include the x,y, and z of the edge nodes for the local processor.
  ...
  // The statement right above is true if we have the number of subdivisions in all directions, otherwise we have the following:
  totalNumEdgesNodes = totalNumEachEdgesNodes[0]*4 + totalNumEachEdgesNodes[1]*4 +totalNumEachEdgesNodes[2]*4;
  
  global_Edges_DOFs_Vector.resize(3,std::vector<unsigned int>(totalNumEdgesNodes,0));
  global_Edges_DOFs_Coord_Vector.resize(3,std::vector<double>(totalNumEdgesNodes,0));
  ...
  local_Edges_DOFs_Vector.resize(3,std::vector<unsigned int>(totalNumEdgesNodes,0));
  local_Edges_DOFs_Coord_Vector.resize(3,std::vector<double>(totalNumEdgesNodes,0));

  global_Edges_DOFs_Vector_Array.resize(3,std::vector<unsigned int>(totalNumEdgesNodes,0));
  global_Edges_DOFs_Coord_Vector_Array.resize(3,std::vector<double>(totalNumEdgesNodes,0));
  ...
  if (size_dof_Edge_processor>0){
   ...
   for(unsigned int i = 0; i < (totalNumEdgesNodes); i++){
...
//Here, we assume that for each node, dof=1 and dof=2 are (dof=1)=(dof=0)+1 and (dof=2)=(dof=0)+2.
  for(unsigned int i = 0; i < totalNumEachEdgesNodes[0]; i++){
...
for(unsigned int i = 0; i < totalNumEachEdgesNodes[1]; i++){
...
for(unsigned int i = 0; i < totalNumEachEdgesNodes[2]; i++){
...
if ((edges_Constraints_Matrix[j][0] < 12) && (edges_Constraints_Matrix[j][1] < 12)){
    temp_edge_subdiv = 0; // subdivions on EX edges
    }
    else if ((edges_Constraints_Matrix[j][0] < 24) && (edges_Constraints_Matrix[j][1] < 24) && (edges_Constraints_Matrix[j][0] > 11) && (edges_Constraints_Matrix[j][1] > 11)){
    temp_edge_subdiv = 1; // subdivions on EY edges
    }
    else if ((edges_Constraints_Matrix[j][0] < 36) && (edges_Constraints_Matrix[j][1] < 36) && (edges_Constraints_Matrix[j][0] > 23) && (edges_Constraints_Matrix[j][1] > 23)){
    temp_edge_subdiv = 2; // subdivions on EZ edges
    }
    else{
    std::cout << "There is a problem with Edges Constraints loop" << std::endl;
    std::exit(1);
    }
    // Now the size of the next for loop will depend on which type of edge we are (EX, EY or EZ)
    for(unsigned int i = 0; i < totalNumEachEdgesNodes[temp_edge_subdiv]; i++){
